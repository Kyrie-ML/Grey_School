...
export function GalleryGrid({ preview = false }: { preview?: boolean }) {
  ...
  const shown = preview ? gallery.slice(0, 5) : filtered;
  ...
  return <>
    {!preview && <div className="mb-12 flex flex-wrap gap-x-6 gap-y-2 border-y border-midnight/15 py-3" aria-label="Filter gallery">{filters.map(item => <button key={item} onClick={() => { setFilter(item); setActive(null); }} aria-pressed={filter === item} className={`focus-ring min-h-10 border-b text-[.65rem] font-bold uppercase tracking-[.13em] transition ${filter === item ? "border-growth text-growth" : "border-transparent text-slate hover:text-midnight"}`}>{item}</button>)}</div>}
    <div className={`grid gap-3 ${preview ? "auto-rows-[220px] grid-cols-2 md:grid-cols-4" : "auto-rows-[230px] grid-cols-1 sm:grid-cols-2 lg:grid-cols-3"}`}>{shown.map((item, index) => <motion.button key={`${filter}-${item.src}`} initial={reduced ? false : { opacity: .5, y: 18 + index * 3, clipPath: "inset(10% 0 16% 0)" }} whileInView={{ opacity: 1, y: 0, clipPath: "inset(0% 0 0% 0)" }} viewport={{ once: true, amount: .15 }} transition={{ duration: reduced ? 0 : .68, delay: index * .08, ease: [0.22, 1, 0.36, 1] }} onClick={() => setActive(index)} aria-label={`Open image: ${item.alt}`} className={`focus-ring group relative overflow-hidden text-left ${preview ? (index === 0 ? "col-span-2 row-span-2" : index === 1 ? "row-span-2" : "") : `${item.span === "wide" ? "sm:col-span-2" : ""} ${item.span === "tall" ? "row-span-2" : ""}`}`}><Image src={item.src} alt={item.alt} fill sizes="(min-width:1024px) 50vw, 100vw" className="object-cover transition duration-700 group-hover:scale-[1.03]" /><span className="absolute inset-0 bg-gradient-to-t from-midnight/75 via-transparent to-transparent opacity-50 transition group-hover:opacity-85" /><span className="absolute bottom-5 left-5 translate-y-2 text-[.65rem] font-bold uppercase tracking-[.15em] text-white opacity-80 transition group-hover:translate-y-0 group-hover:opacity-100">{item.category}</span></motion.button>)}</div>
